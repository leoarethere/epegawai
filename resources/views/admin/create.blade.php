...
<div class="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-md">
    ...
    <div class="flex justify-between items-start mb-8 border-b pb-4">
        <div>
            <h2 class="text-xl font-bold text-gray-800 mb-1">Tambah Pegawai Baru</h2>
            <p class="text-sm text-gray-500">Silahkan isi data pegawai baru dengan lengkap.</p>
        </div>
    ...
    <form action="{{ route('admin.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="flex flex-col md:flex-row gap-8 mb-8">
            {{-- Foto Profil (Layout Lingkaran Sesuai Edit) --}}
            ...
            <div class="w-full md:w-3/4 space-y-5">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" class="w-full border border-gray-300 rounded-md px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-200 focus:border-blue-500 transition shadow-sm" required>
                </div>
                <div class="grid grid-cols-2 gap-5">
                    <div>
                        <label class="block mb-1 font-medium text-gray-700 text-sm">NIK (KTP) <span class="text-red-500">*</span></label>
                        <input type="text" name="nik" value="{{ old('nik') }}" maxlength="16" class="w-full border border-gray-300 rounded-md px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-200 focus:border-blue-500 transition shadow-sm" placeholder="16 Digit" required>
                    </div>
                    <div>
                        <label class="block mb-1 font-medium text-gray-700 text-sm">NIP (Opsional)</label>
                        <input type="text" name="nip" value="{{ old('nip') }}" maxlength="18" class="w-full border border-gray-300 rounded-md px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-200 focus:border-blue-500 transition shadow-sm" placeholder="18 Digit">
                    </div>
                </div>

                <div class="bg-blue-50 p-4 rounded-md border border-blue-200 mt-2">
                    <div class="flex items-start">
                        <i class="fas fa-info-circle text-blue-600 mt-0.5 mr-2"></i>
                        <div>
                            <p class="font-bold text-blue-800 text-xs uppercase mb-1">Password Default</p>
                            <p class="text-sm text-blue-700 leading-tight">Password login akan otomatis diset sesuai dengan <b>NIK</b> yang diinputkan.</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-4">
            <div>
                ...
                <input type="text" name="tempat_lahir" value="{{ old('tempat_lahir') }}" class="w-full border border-gray-300 rounded-md px-3 py-2.5 text-sm shadow-sm">
            </div>
            <div>
                <label class="block mb-1 font-medium text-gray-700 text-sm">Tanggal Lahir</label>
                <input type="date" name="tanggal_lahir" value="{{ old('tanggal_lahir') }}" class="w-full border border-gray-300 rounded-md px-3 py-2.5 text-sm shadow-sm">
            </div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-4">
            <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
            <div>
                ...
                <input type="text" name="jabatan" value="{{ old('jabatan') }}" class="w-full border border-gray-300 rounded-md px-3 py-2.5 text-sm shadow-sm">
            </div>
            <div>
                <label class="block mb-1 font-medium text-gray-700 text-sm">Bagian / Divisi</label>
                <input type="text" name="bagian" value="{{ old('bagian') }}" class="w-full border border-gray-300 rounded-md px-3 py-2.5 text-sm shadow-sm">
            </div>
        </div>
        
        <div class="mb-5">
            <label class="block mb-1 font-medium text-gray-700 text-sm">No Telepon / WA <span class="text-red-500">*</span></label>
            <input type="number" name="telepon" value="{{ old('telepon') }}" class="w-full border border-gray-300 rounded-md px-3 py-2.5 text-sm shadow-sm" required>
        </div>
        
        <div class="mb-5">
            <label class="block mb-1 font-medium text-gray-700 text-sm">Alamat Lengkap (KTP)</label>
            <textarea name="alamat_ktp" rows="2" class="w-full border border-gray-300 rounded-md px-3 py-2.5 text-sm shadow-sm">{{ old('alamat_ktp') }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <input type="url" name="doc_link_sk" value="{{ old('doc_link_sk') }}" 
